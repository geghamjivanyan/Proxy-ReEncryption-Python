...
@typing.no_type_check
def _get_ec_group_by_curve_id(curve_id: int):
    """
    Returns the group of a given curve via its OpenSSL id. This must be freed
    after each use otherwise it leaks memory.
    """
    group = backend._lib.EC_GROUP_new_by_curve_name(curve_id)
...
def _get_new_EC_POINT(curve: 'Curve'):
    ...
    If __curve_id is provided, it retrieves the group from the curve provided.
    """
